...
public class JwtTokenProvider {
    ...
    private String secretKey = "secretKey-test-jwt-spring-boot-create-token";
    ...
    // 객체 초기화, secretKey 를 Base64로 인코딩 한다.
    @PostConstruct
    protected void init(){
    ...
    public String createToken(String userPk, List<String> roles){
        ...
        Date now = new Date();
        return Jwts.builder()
                .setIssuer("/cokes-public-api")
                .setClaims(claims) // 정보 저장
                .setIssuedAt(now) //토큰 발행 시간 정보
                .setExpiration(new Date(now.getTime() + tokenValidTime)) //set Expire Time
                .signWith(key, signatureAlgorithm) //사용할 암호화 알고리즘과 signature 에 들어갈 secret 값 세팅
                .compact();
    }
...
//        return Jwts.parser().setSigningKey(secretKey).parseClaimsJws(token).getBody().getSubject();
    ...
    public String resolveToken(HttpServletRequest request){
        return request.getHeader("COKES-TOKEN");
    }
